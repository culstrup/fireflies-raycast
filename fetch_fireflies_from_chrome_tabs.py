...
def main():
    ...
    try:
        ...
        logger.info(
            f"Script completed in {total_time:.2f} seconds, copied {len(all_transcripts)} transcripts to clipboard"
        )

        if paste_success:
            print(
                f"FlyCast: Copied and pasted {len(all_transcripts)} Fireflies "
                f"transcripts successfully in {total_time:.2f} seconds."
            )
        else:
            # If paste didn't work, it's still in the clipboard
            print(
                f"FlyCast: Copied {len(all_transcripts)} Fireflies transcripts "
                f"to clipboard in {total_time:.2f} seconds. "
